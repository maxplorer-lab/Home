...
// ── Notification channels (one per person, app-wide) ────────────
// The channel belongs to the PERSON, not to a module: a phone follows one
// ntfy topic, so both Sompitra's finance events and W.A.Y's tracking events
// are pushed to the channels listed here. See migrations-home/0002.

/** Topic alphabet: URL-safe, no look-alikes (0/O, 1/l). */
const TOPIC_ALPHABET = 'abcdefghijkmnpqrstuvwxyz23456789'

/** A channel is a bearer secret — anyone who knows it can subscribe — so it
 *  is generated from crypto randomness and never chosen by hand. */
...
/** Everyone's channel — the admin view (and what a push iterates). */
export async function listNtfyChannels(db: D1Database): Promise<NtfyChannel[]> {
  const out = await db
    .prepare(
      `SELECT id, username, display_name, ntfy_topic, ntfy_topic_set_at
...
/**
 * Adopt the channels that already exist in W.A.Y.
 *
 * Before this migration W.A.Y owned a topic per person (way-db
 * `users.ntfy_topic`). A phone is still following that topic, so throwing it
 * away would silently kill every notification — instead each one is copied
 * onto the matching home-db account by username, ONCE, and only when the
 * home account has no channel yet. Idempotent, safe to call on every boot.
 *
 * Returns how many channels were adopted.
 */
export async function adoptWayTopics(env: Env): Promise<number> {
  let adopted = 0
  try {
    ...
    for (const row of way.results ?? []) {
      const res = await env.HOME_DB
        .prepare(
          `UPDATE users SET ntfy_topic = ?, ntfy_topic_set_at = datetime('now')
           WHERE lower(username) = lower(?) AND (ntfy_topic IS NULL OR ntfy_topic = '')`
        )
